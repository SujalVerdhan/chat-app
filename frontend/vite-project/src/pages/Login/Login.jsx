...
await Login(inputs)
  ...
  return (
    <div className="bg-transparent w-96 max-w-md bg-gray-500 rounded-md bg-clip-padding backdrop-filter backdrop-blur-md bg-opacity-0 border border-gray-100
     shadow-2xl  min-w-80  ">
    <div className='flex flex-col  text-gray-100 p-4 w-full'>
...
<input type="text" placeholder="Type here" onChange={(e)=>setInputs({...inputs,userName:e.target.value})} className="input input-bordered w-full h-10" />
</div>
...
<input type="text" placeholder="Type Your Password" onChange={(e)=>setInputs({...inputs,password:e.target.value})} className="input input-bordered w-full h-10" />
</div>

<Link to="/signup" className=' p-2 text-sm hover:underline hover:text-blue-600 mt-2 inline-block'  >Don't Have an account?</Link>
<button className="btn btn-block btn-sm-2 mt-2 p-0 h-6">{loading?<span className="loading loading-infinity loading-md"></span>:"Login"}</button>
</form>
</div>
    </div>
  )
